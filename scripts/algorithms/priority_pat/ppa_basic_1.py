...
class PPABasic:
    ...
    def utility(self, path):
        ...
        if path[-1] in self.priority_nodes:
            reward += self.l_prior * self.graph.nodes[path[-1]]['idleness']
    ...
    def callback_ready(self, req):
        ...
        Verify algorithm
        '''
        if req.algo == 'ppa_basic_1' and self.ready:
...
if __name__ == '__main__':
    rospy.init_node('ppa_basic_1', anonymous = True)

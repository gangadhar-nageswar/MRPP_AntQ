# -*- coding: utf-8 -*-
"""walks_train_ford_fulkerson.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1e7UzDNI0E1VSyo3ZlwV2eEVwY3JCIAwz
"""
...
class Ant_Q:
    ...
    def create_biGraph(self, curr_nodes):
        # bipartite graph

        b = curr_nodes.copy()
        a = self.init_nodes.copy()
        print("a:", a)
        ...
        for ai in a:
            for bj in b:
                if ai != 'd' and bj != 's':
                    g_temp.add_edge(bj, ai, capacity=1.0)
                    g_temp[bj][ai]['name'] = '{}_to_{}'.format(bj, ai)
                    g_temp[bj][ai]['length'] = self.graph[ai][bj]["length"]
    ...
    def return_to_base(self, bi_g, le):
        curr_nodes = [n for n, d in bi_g.nodes(data=True) if d["bipartite"] == 'current']
        # des_nodes = [n for n, d in bi_g.nodes(data=True) if d["bipartite"] == 'end']
        des_nodes = self.init_nodes.copy()
        des_nodes.append('d')
        ...
        edges = {}
        at_des_nodes = list(set(des_nodes).intersection(set(curr_nodes)))
        for nod in at_des_nodes:
            edges[nod] = nod
            des_nodes.remove(nod)
            curr_nodes.remove(nod)
        ...
        for j, bj in enumerate(curr_nodes):
            for ai in des_nodes:
                le_temp = le[j] + self.graph[bj][ai]["length"]
                walks.append(["{}_{}".format(bj, ai), le_temp])
    ...
    def run(self):
        ...
        # return to base
        return_graph = self.create_biGraph(cur_nodes)
        print(list(return_graph.edges()))
        final_edges = self.return_to_base(return_graph, le)         # final edges is a dictionary
        final_nodes = []
        
        if final_edges != None:
            for cn in cur_nodes:
                dn = final_edges[cn]
                final_nodes.append(dn)
    ...
    #     # final_nodes = ['0' for _ in self.agents]
